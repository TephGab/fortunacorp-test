          ...
          <ol class="relative border-s border-gray-200 dark:border-gray-700 d-flex justify-content-between mb-1 bg-gray-100 hover:bg-gray-200" v-for="(activity, index) in sortedActivities" :key="index">
              ...
              <table class="table text-sm">
                ...
                <tbody>
                  <tr v-if="activity.operation == 'transfert_si' || activity.operation == 'transfert'">
                   <!-- Capital part for transfert -->
                    <td class="text-nowrap text-right"> {{ formatDecimalNumber(0) }} </td>
                    <td class="text-nowrap text-right">
                      <div>{{ formatDecimalNumber(getLastEntryFromHistory(activity.transaction_activits, 'new_agent_capital') - getLastEntryFromHistory(activity.transaction_activits, 'current_agent_capital')) }} RD$</div>
                    </td>
                    <td class="text-nowrap text-right fw-bold">
                      <div>{{ formatDecimalNumber(getLastEntryFromHistory(activity.transaction_activits, 'new_agent_capital')) }} RD$</div>
                    </td>
                   <!-- Capital part for transfert end -->
                  </tr>
                  <tr v-else>
                    <!-- Capital Part for reception -->
                    <td class="text-nowrap text-right"> {{ formatDecimalNumber(0) }} </td>
                    <td class="text-nowrap text-right">
                      <div>{{ formatDecimalNumber(getLastEntryFromHistory(activity.transaction_activits, 'new_agent_capital') - getLastEntryFromHistory(activity.transaction_activits, 'current_agent_capital')) }} RD$</div>
                    </td>
                    ...
                    <!-- End Capital part for reception end -->
                  </tr>
                </tbody>
                <!-- End Debit / Credit -->
              </table>
            ...
            <li class="text-muted" v-if="activity.table == 'agent_loan_repays'">
              ...
              <tr>
                ...
                <td class="text-nowrap text-right fw-bold">
                  <div>{{ formatDecimalNumber(getLastEntryFromHistory(activity.agent_loan_repay_activits, 'new_agent_capital')) }} RD$</div>
                </td>
              </tr>
              </tbody>
              </table>
            ...
            <!-- AGENT DEPOSITS LI -->
             <li class="mb-2 ms-6" v-if="activity.table == 'agent_deposits'">
                      ...
                      <h3 class="flex items-center mb-1 text-sm font-semibold text-gray-500 dark:text-dark text-uppercase">
                        ...
                        <span class="text-capitalize text-sm fw-normal ml-1"> ({{ activity.operation }})</span>
                      </h3>
                      <p class="mb-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                        <span>
                       ...
                       <tbody>
                          <tr>
                            ...
                            <td class="text-nowrap text-right fw-bold" v-if="activity.agent_deposit_activits">
                              <div>{{ formatDecimalNumber(getLastEntryFromHistory(activity.agent_deposit_activits, 'new_agent_capital')) }} RD$</div>
                            </td>
                          </tr>
                        </tbody>
                      </table>
