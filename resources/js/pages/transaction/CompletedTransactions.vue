<template>
        ...
        <div class="col-md-12">
                ...
                <table class="table text-sm">
                  <thead>
                    ...
                    <tr>
                      ...
                      <th class="text-nowrap text-center" v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'system-engineer'"> Operator </th>
                      <th class="text-nowrap text-center">Date</th>
                      <th class="text-nowrap text-center">Actions</th>
                    </tr>
                  </thead>
                  <tbody v-for="transaction in completedTransactions" :key="transaction.id">
                    <tr :class="[minutesSincePending(transaction.created_at, transaction.status) > 30 ? longPendingClass : 'hover:bg-gray-300', 'text-nowrap']">
                        ...
                        <div class="sparkbar" data-color="#00a65a" data-height="20">
                          <div v-if="transaction.operation == 'transfert' || transaction.operation == 'transfert_si'">
                            <div class="text-gray-600">
                              ...
                              <div class="text-muted text-xs text-italic text-right" v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'operator' || userrole.name == 'system-engineer'"> Business acc.</div>
                            </div>
                            <!-- <div class="fw-bolder text-center border-top m-0" v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'operator' || userrole.name == 'system-engineer'">
                              <p class="border-top m-0 w-100 border-gray-600"></p>
                            </div> -->
                            <div class="text-secondary border-top border-gray-600" v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'operator' || userrole.name == 'system-engineer'">
                      ...
                      <td v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'system-engineer'" class="text-nowrap text-xs text-red text-uppercase align-middle" v-if="transaction.operator_id == 1">
                        No Operator
                      </td>
                      <td v-show="userrole.name == 'super-admin' || userrole.name == 'admin' || userrole.name == 'system-engineer'" class="text-nowrap text-xs text-dark text-uppercase align-middle text-center" v-else>
          ...
          <div class="row">
            <div class="col-md-12 col-sm-12 col-lg-12 overflow-auto mr-5">
              <Pagination :data="completedTransactions" @pagination-change-page="getCompletedTransactions" class="mr-5" />
            </div>
          </div>
          <!-- /.card -->
        </div>
  ...
  export default {
    ...
    setup(props) {
      ...
      const { getCompletedTransactions, completedTransactions, transaction, getNotifications, notifications, formatDate,
        getDailyRate, dailyrateSale, dailyRateUpdate, transactionSort, isLoading,
        generatePdf, isButtonLoading, setMonthAndYear, agentCancelTransaction } = useTransactions();
      ...
      //   try {
      //     if (form.q_id.length > 0) {
      //       const response = await axios.post("/api/transactions/search", { q_id: form.q_id, status: 'completed' });
      //       completedTransactions.value = response.data;
      //     } else {
      //       getCompletedTransactions();
      //     }
      //   } catch (error) {
